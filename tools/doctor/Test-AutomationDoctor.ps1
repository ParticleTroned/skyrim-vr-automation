...
try {
    ...
    $fixtureCheck = @($inspected.checks | Where-Object name -eq 'prime-profile-world-entry')
    if ($fixtureCheck.Count -ne 1 -or $fixtureCheck[0].status -ne 'fail' -or [string]::IsNullOrWhiteSpace([string]$fixtureCheck[0].data.exampleManifestPath)) { throw "Doctor did not fail an unqualified prime-profile world-entry save: $($fixtureCheck | ConvertTo-Json -Depth 12 -Compress)" }
