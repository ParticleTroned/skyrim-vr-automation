...
## Follow-up independent-review reconciliation — 2026-08-29

The later package `CSXPR-BROWSER-MSG-000006_AUTOMATION-NEW-WORK-REVIEW-RESPONSE_20260829.zip`
correctly distinguished caught-exception rollback from restart-safe recovery.
It reported 15 findings against PR11/PR12 source and 9 findings against a
separate local quiet-window profiler experiment. The current feedback mailbox
was also checked after these repairs: 74 records were scanned and no `new`,
`triaged`, or `accepted` record remained.

The findings that applied to repository main have the following disposition:

| Finding | Current disposition | Merged evidence |
| --- | --- | --- |
| PR11-B01 | Readiness arguments are restricted to schema-valid, tool-specific non-mutating probes; arbitrary explicit mutation is rejected | PR #12, `c92c2e2` |
| PR11-B02 | Profile operations use a canonical target-owned lock/journal and reconcile nonterminal exact preimage/result state before planning | PR #17, `d174f33`; 30 focused assertions |
| PR11-B03 | Workspace create/resume/retire and selected-profile subtransactions recover before command-specific reads | PR #17, `d174f33`; 61 focused assertions including fabricated restart states |
| PR11-B04 | Shader-cache swaps serialize by canonical live target and recover interrupted displacement/activation phases | PR #16, `0d9093f`; 29 transaction assertions |
| PR11-B05 | SteamVR/OpenVR transactions use one canonical target lock and authoritative journal independent of caller evidence | PR #14, `4d85490`; 56 null-control assertions |
| PR11-B06 | Head-pose install/upgrade uses a separate install lock and restart recovery with exact registration/install verification | PR #15, `54c7494`; 20 focused assertions |
| PR11-B07 | Profiler captures hold a bounded runtime lease, verify every response identity, and recover a dead capture's same-runtime prior state without mutating a replacement process | PR #18 plus the follow-up recovery change; 14 focused assertions |
| PR11-H01 | Catalogue and transaction paths share one file/byte/depth/deadline/reparse-bounded inventory primitive | PR #16, `0d9093f`; 28 catalogue assertions |
| PR11-H02 | SteamVR polling uses a deadline-charged incremental bounded-byte log tail | PR #14, `4d85490` |
| PR11-H03 | Profiler capture has one caller-visible total deadline and a reserved restoration budget | PR #18, `c56499b`; bounded-deadline fixture |
| PR12-B01 | Unknown nonempty content remains unready and only recognized positive contracts qualify readiness | PR #12, `c92c2e2`; negative readiness fixtures |
| PR12-H01 | Static save/profile evidence is named `integrityVerified`; `runtimeQualified` remains false without separate runtime evidence | PR #12, `c92c2e2` |
| PR12-H02 | Profile/save traversal, hashing, copy, and verification share explicit count/byte/depth/deadline limits | PR #12, `c92c2e2` |
| PR12-H03 | Selected-profile replacement has immediate preimage CAS plus deterministic restart recovery through its parent workspace transaction | PR #12 and PR #17 |
| PR12-M01 | Clone time is reported as `cloneVerifiedUtc`; runtime qualification time can come only from runtime evidence | PR #12, `c92c2e2` |

The nine `LOCAL-*` findings reviewed files named
`Measure-SkyrimQuietWindow.ps1` and `Compare-SkyrimPerformance.ps1`. That
experimental delta was rejected and those files were never merged into this
repository. They are therefore not silently marked resolved: their aligned
measurement-window, comparability, process-counter identity, quiet-scene,
boundedness, atomic-evidence, validation, and test requirements remain the
mandatory design gate if that experiment is revived. Applicable controls in
the maintained CSX profiler collector were strengthened, but this does not
convert the rejected quiet-window measurements into valid evidence.

After the follow-up profiler recovery lands, the deterministic Windows toolset
qualification is 16/16 green. The review's stronger destructive-process test
request—terminating a real controller at every persisted phase and recovering
against disposable live fixtures—remains a release-qualification activity, not
a claim made by the synthetic suite.
