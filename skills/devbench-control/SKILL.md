...
8. Keep readiness recovery on the selected lane. On the direct lane, repeat
   ...
   `serviceReady` with its schema-derived read-only probe.
...
12. Set `-MaxTransientRetries 0` for ownership-bearing actions. If a response
    is lost, inspect the existing owner on the same lane; never replay the
    action or clean up its evidence prematurely.
13. Preserve `invocationEvidencePath` for failed bundled calls. Never run
    `tfc 1` unless the user explicitly authorizes a crash-risk experiment.
14. For direct or nested scenario `game` mutations on the bundled lane, pass
