...
try {
    ...
    classifiedRecord.presentationPath.allowedPresentationStretch.completedFrames = 7;
    classifiedRecord.presentationPath.allowedPresentationStretch.traceComplete = false;
    classifiedRecord.acceptance.gates[1].observed.completedFrames = 7;
    ...
    write(classifiedFile, classifiedReceipt);
    const missingFrameHealth = load(classified).health.passes[0];
    assert.equal(missingFrameHealth.evidenceStatus, "COMPLETE");
    assert.equal(missingFrameHealth.healthStandard, "NOT_MET");
    classifiedRecord.presentationPath.allowedPresentationStretch.completedFrames = 6;
    classifiedRecord.presentationPath.allowedPresentationStretch.traceComplete = true;
    classifiedRecord.acceptance.gates[1].observed.completedFrames = 6;
    classifiedRecord.acceptance.gates[1].passed = true;
    classifiedRecord.acceptance.accepted = true;
    classifiedRecord.acceptance.verdict = "pass";
    classifiedRecord.acceptance.failureReasons = [];
    classifiedRecord.acceptance.gates.push({ name: "future_health_gate", passed: false });
    classifiedRecord.acceptance.accepted = false;
    classifiedRecord.acceptance.verdict = "fail";
    classifiedRecord.acceptance.failureReasons = ["future_health_gate"];
    write(classifiedFile, classifiedReceipt);
    assert.equal(load(classified).health.passes[0].healthStandard, "NOT_MET");
    classifiedRecord.acceptance.gates.at(-1).classification = "diagnostic_only";
    write(classifiedFile, classifiedReceipt);
