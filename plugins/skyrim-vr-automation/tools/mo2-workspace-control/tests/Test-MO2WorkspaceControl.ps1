...
try {
    ...
    foreach ($p in @($source, (Join-Path $source 'saves'), $loaderMod, (Join-Path $loaderMod 'SKSE\Plugins'), (Join-Path $ocuMod 'root'), (Join-Path $ocuMod 'SKSE\Plugins'), $csxReleaseMod, $csxDevBenchMod, (Join-Path $synthesisMod 'ShaderCache\Lighting'), (Join-Path $synthesisMod 'backup\previous'), (Join-Path $mo2 'overwrite'), (Join-Path $mo2 'rb'), $sessions, (Join-Path $fixture 'archive'))) { New-Item -ItemType Directory -Path $p -Force | Out-Null }
    ...
    try {
        ...
        if ($reparsePrepare.ok -or @($reparsePrepare.errors | Where-Object { $_ -match 'reparse point' }).Count -ne 1 -or
            ...
            throw 'prepare-source did not reject a reparse-point Overwrite root while preserving caches and the stable profile.'
        }
    }
    finally {
    ...
    try {
        ...
        if ($nestedReparsePrepare.ok -or @($nestedReparsePrepare.errors | Where-Object { $_ -match 'reparse point' }).Count -ne 1 -or
            @(Get-ChildItem -LiteralPath $mods -Directory -Force).Count -ne $modCountBeforeReparse -or
            [Convert]::ToBase64String([IO.File]::ReadAllBytes((Join-Path $source 'modlist.txt'))) -cne [Convert]::ToBase64String($sourceModListBeforeReparse)) {
            throw 'prepare-source did not reject a nested Overwrite reparse point while preserving caches and the stable profile.'
        }
    }
    finally {
    ...
    $preTaskCacheTree = Get-TestProfileFingerprint (Join-Path $mo2 'overwrite\ShaderCache')
    ...
    if (-not $created.data.runtimeOutput.cachePathExistedBefore -or -not $created.data.runtimeOutput.backupPathExistedBefore) { throw 'Workspace did not record the pre-existing cache and backup states.' }
    ...
    if (-not (Test-Path -LiteralPath $created.data.runtimeOutput.cachePath -PathType Container) -or (Get-TestProfileFingerprint $created.data.runtimeOutput.cachePath) -cne $preTaskCacheTree) { throw 'Cache completion did not restore the exact pre-task ShaderCache tree.' }
